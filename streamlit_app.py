...
if page == "1. Data Retrieval":
    ...
    if st.button("Search Ligands") and query:
        with st.spinner("Searching ChEMBL..."):
            ...
            if target_id:
                # FIXED: Only requesting fields we actually need to ensure 'target_organism' is included
                ...
                if not df.empty:
                    # Rename 'target_organism' to 'organism' for the ML section
    ...
    if st.session_state.ligand_df is not None:
        ...
        # FIXED: Organism is now explicitly included in the display columns
    ...
    if st.button("Prepare 3D"):
        ...
        if mol:
            ...
            view.addModel(Chem.MolToMolBlock(mol), 'mol'); view.setStyle({'stick':{}}); view.zoomTo(); showmol(view)